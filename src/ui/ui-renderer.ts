...
export function renderSetupScreen(
  ...
  const urlInput = card.querySelector('#setup-url') as HTMLInputElement;
  const proxyInput = card.querySelector('#setup-proxy') as HTMLInputElement;
  const proxySection = card.querySelector('#setup-proxy-section') as HTMLElement;
  const testBtn = card.querySelector('#setup-test') as HTMLButtonElement;
  const skipBtn = card.querySelector('#setup-skip') as HTMLButtonElement;
  const statusDiv = card.querySelector('#setup-status') as HTMLElement;
  const continueBtn = card.querySelector('#setup-continue') as HTMLButtonElement;
  ...
  testBtn.addEventListener('click', async () => {
    ...
    const client = new CatalogueClient(
      { url, proxyUrl: proxyInput.value.trim() || null },
      500
    );
...
export function renderMainInterface(
  ...
  // Wire up URL check
  const urlCheckBtn = container.querySelector('#url-check') as HTMLButtonElement;
  ...
  // Wire up XML check
  const xmlCheckBtn = container.querySelector('#xml-check') as HTMLButtonElement;
  const xmlInput = container.querySelector('#xml-input') as HTMLTextAreaElement;
  xmlCheckBtn.addEventListener('click', () => app.checkXml(xmlInput.value));

  // Wire up batch search
  const batchSearchBtn = container.querySelector('#batch-search-btn') as HTMLButtonElement;
  batchSearchBtn.addEventListener('click', () => {
    const term = (container.querySelector('#batch-search-term') as HTMLInputElement).value;
    const type = (container.querySelector('#batch-resource-type') as HTMLSelectElement).value;
    app.searchBatch(term, type || undefined);
  });

  // Wire up batch UUID
  const batchUuidBtn = container.querySelector('#batch-uuid-btn') as HTMLButtonElement;
  batchUuidBtn.addEventListener('click', () => {
    const text = (container.querySelector('#batch-uuid-input') as HTMLTextAreaElement).value;
...
export function renderRecordReport(
  container: HTMLElement,
  report: RecordReport,
  condensedGuide: string,
  ...
  const copyBtn = summaryBar.querySelector('.btn-copy-main') as HTMLButtonElement;
  const copyMenu = summaryBar.querySelector('.copy-dropdown-menu') as HTMLElement;
  ...
  copyMenu.addEventListener('click', async (e) => {
    const target = (e.target as HTMLElement).closest('.copy-option') as HTMLElement | null;
    if (!target) return;
  ...
  if (report.record.sourceUrl && onRecheck) {
    const recheckBtn = summaryBar.querySelector('.btn-recheck') as HTMLButtonElement;
...
function renderSection(section: SectionReport): HTMLElement {
      ...
      if (entity) {
        ...
        if (passingCount > 0 && nonPassingCount > 0) {
          ...
          toggle.addEventListener('click', (e) => {
            ...
            entityBody.querySelectorAll('.passing-check-hidden').forEach(ch =>
              (ch as HTMLElement).style.display = showing ? '' : 'none'
            );
...
function renderCheckResult(result: CheckResult): HTMLElement {
  ...
  const detail = row.querySelector('.check-detail') as HTMLElement | null;
  if (detail) {
...
export function renderSettingsPanel(
  ...
  const saveCurrentSettings = () => {
    ...
    panel.querySelectorAll('[data-section]').forEach(el => {
      if ((el as HTMLInputElement).checked) {
        settings.enabledSections.push((el as HTMLElement).dataset.section!);
      }
    });
  ...
  const cataloguesDiv = panel.querySelector('#settings-catalogues') as HTMLElement;
  ...
  panel.querySelector('#settings-add-catalogue')?.addEventListener('click', () => {
    ...
    if (addSection) { addSection.remove(); return; }
    ...
    form.querySelector('#new-cat-test')?.addEventListener('click', async () => {
      const url = (form.querySelector('#new-cat-url') as HTMLInputElement).value.trim();
      if (!url) return;
      const proxyUrl = (form.querySelector('#new-cat-proxy') as HTMLInputElement).value.trim() || null;
      const statusEl = form.querySelector('#new-cat-status') as HTMLElement;
      ...
      if (result.cors && result.csw) {
        ...
        if (settings.activeCatalogueIndex < 0) settings.activeCatalogueIndex = 0;
        saveCurrentSettings();
        renderCatalogueList(cataloguesDiv, settings, saveCurrentSettings);
        form.remove();
      }
...
export function renderBatchReport(
  ...
  let selectedIndex = 0;
  function renderSelectedRecord(index: number): void {
    selectedIndex = index;
  ...
  function refreshBatchUi(index: number): void {
    ...
    const row = list.querySelector(`[data-index="${index}"]`) as HTMLElement;
    if (row) {
    ...
    const filterLabels = header.querySelectorAll<HTMLElement>('.filter-checkbox');
    filterLabels.forEach(label => {
      const cb = label.querySelector('input') as HTMLInputElement;
      const filter = cb.dataset.filter as 'pass' | 'warning' | 'error';
...
function renderCatalogueList(
  ...
  container.querySelectorAll('input[name="active-catalogue"]').forEach(radio => {
    radio.addEventListener('change', () => {
      settings.activeCatalogueIndex = parseInt((radio as HTMLInputElement).value, 10);
      onSave();
    });
  });
...
function renderConnectionStatus(container: HTMLElement, result: ConnectionTestResult): void {
  container.innerHTML = `
...
export interface AppController {
  getActiveCatalogue(): { url: string; proxyUrl: string | null } | null;
 ...
 */
function linkifyPids(escapedHtml: string): string {
  ...
  escapedHtml = escapedHtml.replace(
    /\b(\d{4}-\d{4}-\d{4}-\d{3}[\dX])\b/g,
    '<a href="https://orcid.org/$1" target="_blank" rel="noopener" class="pid-link">$1</a>'
  );
...
function countSeverities(sections: SectionReport[]): Record<Severity, number> {
  ...
  for (const s of sections) {
    for (const r of s.results) counts[r.severity]++;
  }
  return counts;
}

function getWorstSeverity(results: CheckResult[]): Severity {
...
function getWorstSeverityForReport(report: RecordReport): Severity {
  const all = report.sections.flatMap(s => s.results);
  return getWorstSeverity(all);
}

function groupByEntity(results: CheckResult[]): Map<string, CheckResult[]> {
