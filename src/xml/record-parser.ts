...
import type {
  ParsedRecord, ParsedCitation, ParsedIdentifier, ParsedAssociatedResource,
  ParsedResponsibleParty, ParsedOrganisation, ParsedIndividual,
  ParsedOnlineResource, ParsedConstraint, TemplatePlaceholder,
  RecordType, SchemaType
} from '../types.js';
import {
  buildNamespaceResolver, detectSchema,
  xpathElement, xpathNodes, xpathText, xpathAttr, extractCodeValue
} from './xpath-helpers.js';

/**
 * Parse an XML Document into a ParsedRecord.
 */
export function parseRecord(doc: Document, sourceUrl?: string): ParsedRecord {
...
function extractUuid(root: Element, schema: SchemaType, resolver: XPathNSResolver): string | null {
...
function extractTitle(root: Element, schema: SchemaType, resolver: XPathNSResolver): string | null {
  if (schema === 'iso19115-3') {
...
function extractScopeCode(root: Element, schema: SchemaType, resolver: XPathNSResolver): string | null {
  if (schema === 'iso19115-3') {
...
function extractParentUuid(root: Element, schema: SchemaType, resolver: XPathNSResolver): string | null {
  if (schema === 'iso19115-3') {
...
function detectRecordType(scopeCode: string | null, title: string | null): {
  recordType: RecordType;
  recordTypeSource: 'scope' | 'title';
} {
  // Primary: scope code
  if (scopeCode) {
    const code = scopeCode.toLowerCase();
    if (code === 'dataset') return { recordType: 'dataset', recordTypeSource: 'scope' };
    if (code === 'fieldsession') return { recordType: 'project', recordTypeSource: 'scope' };
    if (code === 'series') return { recordType: 'program', recordTypeSource: 'scope' };
  }
...
function extractCitation(root: Element, schema: SchemaType, resolver: XPathNSResolver): ParsedCitation {
  let citation: Element | null = null;

  if (schema === 'iso19115-3') {
    const dataId = xpathElement(root, 'mdb:identificationInfo/mri:MD_DataIdentification', resolver);
    if (dataId) citation = xpathElement(dataId, 'mri:citation/cit:CI_Citation', resolver);
  } else if (schema === 'iso19139') {
    const dataId = xpathElement(root, 'gmd:identificationInfo/gmd:MD_DataIdentification', resolver);
    if (dataId) citation = xpathElement(dataId, 'gmd:citation/gmd:CI_Citation', resolver);
  }

  if (!citation) return { identifiers: [] };

  const identifiers = extractIdentifiers(citation, schema, resolver);
  return { identifiers };
}

function extractIdentifiers(citationEl: Element, schema: SchemaType, resolver: XPathNSResolver): ParsedIdentifier[] {
...
function extractAssociatedResources(root: Element, resolver: XPathNSResolver): ParsedAssociatedResource[] {
  const dataId = xpathElement(root, 'mdb:identificationInfo/mri:MD_DataIdentification', resolver);
  if (!dataId) return [];
...
function extractResponsibleParties(
  root: Element, schema: SchemaType, resolver: XPathNSResolver,
  section: 'citation' | 'pointOfContact'
): ParsedResponsibleParty[] {
  const parties: ParsedResponsibleParty[] = [];
  ...
  if (schema === 'iso19115-3') {
    ...
    for (const node of rpNodes) {
      ...
      const org: ParsedOrganisation | null = orgName
        ? { name: orgName, partyIdentifiers: [], onlineResources } : null;
      const individual: ParsedIndividual | null = indName
...
function extractOrganisation19115(orgEl: Element, resolver: XPathNSResolver): ParsedOrganisation {
...
function extractIndividual19115(indEl: Element, resolver: XPathNSResolver): ParsedIndividual {
...
function extractPartyIdentifiers(el: Element, resolver: XPathNSResolver): ParsedIdentifier[] {
  const idNodes = xpathNodes(el, 'cit:partyIdentifier/mcc:MD_Identifier', resolver);
  return idNodes.map(node => {
    const idEl = node as Element;
...
function extractOnlineResources19115(orgEl: Element, resolver: XPathNSResolver): ParsedOnlineResource[] {
...
function extractOnlineResources19115Individual(indEl: Element, resolver: XPathNSResolver): ParsedOnlineResource[] {
...
function extractOnlineResources19139(rpEl: Element, resolver: XPathNSResolver): ParsedOnlineResource[] {
...
function extractConstraints(root: Element, schema: SchemaType, resolver: XPathNSResolver): ParsedConstraint[] {
  const constraints: ParsedConstraint[] = [];
  let dataId: Element | null = null;
    ...
    for (const node of constraintNodes) {
      ...
      if (localName === 'MD_LegalConstraints') {
        ...
        for (const n of otherNodes) {
          const text = (n as Element).textContent?.trim();
          if (text) otherConstraints.push(text);
        }
...
function detectTemplatePlaceholders(doc: Document): TemplatePlaceholder[] {
...
function buildLocationPath(node: Node): string {
  const parts: string[] = [];
