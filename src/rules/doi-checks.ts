...
export const doiChecks: RuleSection = {
  id: 'doi',
  name: 'DOI Checks',
  ...
  async check(record: ParsedRecord, context: CheckContext): Promise<CheckResult[]> {
    ...
    if (record.recordType !== 'dataset') return results;

    // Find DOI identifier: codeSpace = "doi.org" or contains DOI pattern
    const doiId = record.citation.identifiers.find(id =>
      id.codeSpace?.toLowerCase() === 'doi.org'
    );
...
function extractBareDoi(value: string): string {
