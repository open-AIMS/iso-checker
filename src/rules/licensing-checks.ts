...
export const licensingChecks: RuleSection = {
  id: 'licensing',
  ...
  async check(record: ParsedRecord, _context: CheckContext): Promise<CheckResult[]> {
    if (record.recordType !== 'dataset') return [];
    ...
    if (legalConstraints.length === 0) return results;
    ...
    const hasCitation = legalConstraints.some(c =>
      c.otherConstraints.some(text => text.length > 50)
    );
