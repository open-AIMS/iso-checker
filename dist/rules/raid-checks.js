...
async function checkProjectRaid(record, context) {
    const results = [];
    const raidId = record.citation.identifiers.find(id => id.codeSpace?.toLowerCase() === 'raid.org');
    if (!raidId) {
        results.push({
            id: 'raid-present',
...
async function checkDatasetRaid(record, context) {
    const results = [];
    // Find associated resource with RAiD
    const raidResource = record.associatedResources.find(ar => ar.identifiers.some(id => id.codeSpace?.toLowerCase() === 'raid.org'));
    if (!raidResource) {
        results.push({
            id: 'raid-present',
...
function checkRaidEncoding(id, prefix, _context) {
    const results = [];
    const code = id.code ?? '';
    // Bare string check
...
//# sourceMappingURL=raid-checks.js.map